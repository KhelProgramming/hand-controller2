...
raw_gesture_queue = queue.Queue(maxsize=5)
...
def ml_thread():
    ...
    while not stop_event.is_set():
        try:
            ...
            while raw_gesture_queue.full():
                try: raw_gesture_queue.get_nowait()
                except queue.Empty: break
            raw_gesture_queue.put((capture_time, gesture))
        except queue.Empty:
            continue
    tracker.close()
...
def logic_thread():
    while not stop_event.is_set():
        try:
            capture_time, gesture = raw_gesture_queue.get(timeout=0.1)
            
            gesture = str(gesture) + "_PROCESSED"
...
def run_benchmark(duration_sec=10):
    print(f"Starting 5-Thread (Theoretical Beast) Benchmark for {duration_sec} seconds...")
    t_io = threading.Thread(target=io_thread, daemon=True)
    t_ml = threading.Thread(target=ml_thread, daemon=True)
    t_logic = threading.Thread(target=logic_thread, daemon=True)
    t_exec = threading.Thread(target=exec_thread, daemon=True)
    
    t_io.start(); t_ml.start(); t_logic.start(); t_exec.start()
    ...
    stop_event.set()
    t_io.join(); t_ml.join(); t_logic.join(); t_exec.join()
    ...
    print("\n=== RESULTS: 5-THREAD SETUP (GIL Test) ===")
