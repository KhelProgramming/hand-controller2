...
def run_benchmark(duration_sec=10):
    print(f"Starting 4-Thread (Sweet Spot) Benchmark for {duration_sec} seconds...")
    t_io = threading.Thread(target=io_thread, daemon=True)
    t_brain = threading.Thread(target=brain_thread, daemon=True)
    t_exec = threading.Thread(target=exec_thread, daemon=True)
    
    t_io.start(); t_brain.start(); t_exec.start()
    ...
    stop_event.set()
    t_io.join(); t_brain.join(); t_exec.join()
    ...
    print("\n=== RESULTS: 4-THREAD SETUP (Sweet Spot) ===")
